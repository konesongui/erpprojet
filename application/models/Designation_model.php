...
class Designation_model extends MY_model {
    ...
    public function gets($id = null) {
        ...
        if (!empty($id)) {
            $query = $this->db->where("id", $id)->get("staff_sanction");
            return $query->row_array();
        } else {
            $query = $this->db->where("is_active", "yes")->get("staff_sanction");
    ...
    function deleteSanction($id) {
        ...
        //=======================Code Start===========================
        $this->db->where("id", $id)->delete("staff_sanction");
        $message = DELETE_RECORD_CONSTANT . " On staff sanction id " . $id;
        $action = "Delete";
        $record_id = $id;
    ...
    function addSanction($data) {
        ...
        if (isset($data["id"])) {
            ...
            $this->db->where("id", $data["id"])->update("staff_sanction", $data);
            $message = UPDATE_RECORD_CONSTANT . " On  staff sanction id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $this->db->insert("staff_sanction", $data);
            $id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On  staff sanction id " . $id;
            $action = "Insert";
            $record_id = $id;
            $this->log($message, $record_id, $action);
            //======================Code End==============================
