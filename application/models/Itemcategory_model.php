...
class Itemcategory_model extends MY_Model
{
    ...
    {
        $this->db->select()->from('caisse_category');
        if ($id != null) {
            $this->db->where('id', $id);
        } else {
            $this->db->order_by('id');
    ...
    public function removes($id)
    {
        ...
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('caisse_category');
        $message   = DELETE_RECORD_CONSTANT . " On caisse category id " . $id;
        $action    = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
        //======================Code End==============================
    ...
    public function adds($data)
    {
        ...
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('caisse_category', $data);
            $message   = UPDATE_RECORD_CONSTANT . " On  caisses id " . $data['id'];
            $action    = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $this->db->insert('caisse_category', $data);
            $insert_id = $this->db->insert_id();
            $message   = INSERT_RECORD_CONSTANT . " On caisses id " . $insert_id;
            $action    = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
            //======================Code End==============================
